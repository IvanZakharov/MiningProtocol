...
contract Management {
    ...
    uint256 public startPriceForMNPRT = 10000;
    uint256 public maxMNPRTMarkup = 40000;
    ...
    /**
     * @dev Exchanges the MNP tokens to MNPRT tokens. Works up to 48 MNPRT
     * tokens at one-time buying. Should call after approving MNP tokens to
     * manager address.
     */
    function buyMNPRT() external {
        ...
        uint256 _mnprtAmount = _allowed.div(_currentPrice);
        _allowed = _mnprtAmount.mul(_currentPrice);

        require(token.transferFrom(msg.sender, fund, _allowed));

        for (uint256 i = 0; i < _mnprtAmount; i++) {
    ...
    /**
     * @dev Sends the daily mining reward to MNPRT holder.
     */
    ...
    */
    ...
    /**
     * @dev Returns the MNPRT token amount of holder.
     */
    ...
    /**
     * @return uint256 the current MNPRT token price in MNP (without decimals).
     */
    function getPrice(uint256 _timestamp) public view returns(uint256) {
        ...
        if (_raising > maxMNPRTMarkup) _raising = maxMNPRTMarkup;
        return (startPriceForMNPRT + _raising) * 10 ** 18;
    }
